...
// A propósito NO importamos `@/lib/env`: ese módulo valida TODO el env de forma
// eager al cargarse. Como el rate-limiter lo importan muchas rutas (auth,
// envíos, checkout), colgarnos de él haría que todas exijan el env completo en
// build-time. Acá leemos solo las 3 variables propias, directo de process.env.
const RATE_LIMIT_DRIVER: "postgres" | "upstash" =
  process.env.RATE_LIMIT_DRIVER === "upstash" ? "upstash" : "postgres";
 ...
 *
 * A propósito sin el SDK `@upstash/ratelimit`: la API REST es HTTP plano y no
 * cuesta una dependencia. Solo se usa si RATE_LIMIT_DRIVER === "upstash".
 *
...
async function consumeUpstash(rule: RateLimitRule): Promise<RateLimitResult> {
  if (!UPSTASH_URL || !UPSTASH_TOKEN) {
    throw new Error("RATE_LIMIT_DRIVER=upstash pero faltan UPSTASH_REDIS_REST_URL/TOKEN");
  }
...
export async function consume(rule: RateLimitRule): Promise<RateLimitResult> {
  try {
    ...
    console.error(`[rate-limit] Motor caído (${RATE_LIMIT_DRIVER}), bloqueando "${rule.key}":`, error);
